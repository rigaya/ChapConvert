...
private:
    ...
    const char *m_filepath = nullptr;             //読み込むチャプターファイル
...
public:
    ...
    int read_file(const char *filepath, uint32_t code_page, double duration);

    //チャプターファイルの書き出し
    //out_filepath...出力先
    //out_chapter_type...出力するチャプターの種類
    //nero_in_utf8...出力するチャプターがneroの場合に、utf-8で出力する
    int write_file(const char *out_filepath, ChapType out_chapter_type, bool nero_in_utf8);
