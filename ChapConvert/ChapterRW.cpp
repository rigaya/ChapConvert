...
//ボム文字かどうか、コードページの判定
uint32_t check_bom(const void* chr) {
...
BOOL fix_ImulL_WesternEurope(uint32_t *code_page) {
...
uint32_t ChapterRW::file_code_page() {
 ...
 int ChapterRW::read_file(const char *filepath, uint32_t code_page, double duration) {
    init();
...
int ChapterRW::write_chapter_apple(const char *out_filepath) {
    int sts = AUO_CHAP_ERR_NONE;
...
int ChapterRW::write_chapter_nero(const char *out_filepath, bool utf8) {
    int sts = AUO_CHAP_ERR_NONE;
    std::ofstream ostream(out_filepath, std::ios::out | std::ios::binary);
    if (!ostream.good()) {
        sts = AUO_CHAP_ERR_FILE_OPEN;
...
int ChapterRW::write_file(const char *out_filepath, ChapType out_chapter_type, bool nero_in_utf8) {
...
int ChapterRW::overwrite_file(ChapType out_chapter_type, bool nero_in_utf8) {
    int sts = AUO_CHAP_ERR_NONE;
    std::string temp_file = m_filepath;
    temp_file += ".tmp";
    for (int i = 0; PathFileExists(temp_file.c_str()); i++) {
        temp_file = strsprintf("%s.tmp%d", m_filepath, i);
    }
    ...
    if (AUO_CHAP_ERR_NONE != sts) {
        ...
        remove(m_filepath);
        rename(temp_file.c_str(), m_filepath);
    }
    return sts;
}
...
int ChapterRW::read_chapter_apple() {
    ...
    if (tinyxml2::XML_NO_ERROR != xml.LoadFile(m_filepath)) {
